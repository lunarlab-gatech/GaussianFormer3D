...
def main(local_rank, args):
    ...
    miou_metric = MeanIoU(
        list(range(1, 8)),
        8, 
        ['grass', 'tree', 'bush', 'puddle', 'mud', 'barrier', 'rubble'],
         True, 8, filter_minmax=False)
    miou_metric.reset()
    ...
    while epoch < max_num_epochs:
        ...
        with torch.no_grad():
            for i_iter_val, data in enumerate(val_dataset_loader):
                ...
                with torch.cuda.amp.autocast(amp):
                    result_dict = my_model(imgs=input_imgs, points=input_points, lidar_feature_maps=input_lidar_features, dpt=input_dpt, anchor_points=input_anchor_points, metas=data) ## Lingjun
