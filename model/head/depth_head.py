...
# from mmdet.models import HEADS # error
from mmseg.models import HEADS
# from mmcv.runner import force_fp32, auto_fp16 # error
...
class DepthHead_GTDpt(DepthHead):
    ...
    def forward(self, x, x_cam=None, gt_dpt=None, return_dpt=False):
        # x_cam: lidar2image, (bs, num_cam, 16)
        # x: List of (B, N, C, H, W), (128, 232, 400), (128, 116, 200), (128, 58, 100), (128, 29, 50)
        # gt_dpt: (B*N, 1, H, W)
        # indice_layer: 2
        # downsample_factor: 32
        batch_size, num_cam, C, H, W = x[self.indice_layer].shape
        # before input, gt_dpt shape: (B, N, H, W)
        dpt_dist = self.get_gt_dpt_dist(gt_dpt.view(batch_size, num_cam, *gt_dpt.shape[1:]).squeeze(2)).permute(0,3,1,2)
        # (B*N, d, H/ds, W/ds), here d is already converted to onehot, H/ds=928/32=29, W/ds=1600/32=50
        # get multi scale depth distribution
        dpt_dists = []
        # multi_scales: (128, 232, 400), (128, 116, 200), (128, 58, 100), (128, 29, 50)
        multi_scales = [[*x_scale.shape[3:]] for x_scale in x]
        for scale in multi_scales:
            dpt_dist_scale = F.interpolate(dpt_dist, size=scale, mode="nearest")
            dpt_dists.append(dpt_dist_scale)
        # dpt_dists: List of (B*N, d, H, W), (112, 232, 400), (112, 116, 200), (112, 58, 100), (112, 29, 50)
        if not return_dpt:
            return dpt_dists
        # dpt_dist: (B*N, 112, 29, 50)
        # dpt_dists: List of (B*N, 112, H, W), (112, 232, 400), (112, 116, 200), (112, 58, 100), (112, 29, 50)
        return dpt_dist, dpt_dists
    ...
    def get_gt_dpt_dist(self, gt_depths_):
        ...
        gt_depths = gt_depths_.view(
            B * N,
            H // self.downsample_factor, # 928 / 32 = 29
            self.downsample_factor, # 32
            W // self.downsample_factor, # 1600 / 32 = 50
            self.downsample_factor, # 32
            1,
        )  # (B*N, H/ds, ds, W/ds, ds, 1)
        gt_depths = gt_depths.permute(0, 1, 3, 5, 2, 4).contiguous()  # (B*N, H/ds, W/ds, 1, ds, ds)
        ...
        # transform depth to depth channel
        gt_depths = (gt_depths -
                     (self.dbound[0] - self.dbound[2])) / self.dbound[2]
        # give invalid depth a value of 0
        gt_depths = torch.where(
            (gt_depths < self.depth_channels + 1) & (gt_depths >= 0.0), # depth_channels=(58-2)/0.5=112
            gt_depths, torch.zeros_like(gt_depths))
        # transform depth channel to one-hot encoding
        gt_depths = F.one_hot(gt_depths.long(),
        ...
        # final shape: (B*N, H/ds, W/ds, d) = (6, 29, 50, 112)
        # This is a preprocessed ground truth depth map with flexibility for training the network, 
        # taking into account depth tolerances and normalization.
