...
class LocalAggregator(nn.Module):
    ...
    def forward(
        ...
        points_int = ((pts - self.pc_min) / self.grid_size).to(torch.int) # (6400, 3)
        # print("points_int.shape: ", points_int.shape) # (640000, 3)
        # print("points_int min: ", points_int.min()) # 0
        # print("points_int max: ", points_int.max()) # 202
        # print("points_int[:, 0].min(): ", points_int[:, 0].min()) # 2
        # print("points_int[:, 0].max(): ", points_int[:, 0].max()) # 202
        # print("points_int[:, 1].min(): ", points_int[:, 1].min()) # 0
        # print("points_int[:, 1].max(): ", points_int[:, 1].max()) # 199
        # print("points_int[:, 2].min(): ", points_int[:, 2].min()) # 0
        # print("points_int[:, 2].max(): ", points_int[:, 2].max()) # 24
        # print("points_int.min() >= 0: ", points_int.min() >= 0) # true
        # print("points_int[:, 0].max() < self.H: ", points_int[:, 0].max() < self.H) # false
        # print("points_int[:, 1].max() < self.W: ", points_int[:, 1].max() < self.W) # true
        # print("points_int[:, 2].max() < self.D: ", points_int[:, 2].max() < self.D) # false
        assert points_int.min() >= 0 and points_int[:, 0].max() < self.H and points_int[:, 1].max() < self.W and points_int[:, 2].max() < self.D
        means3D_int = ((means3D.detach() - self.pc_min) / self.grid_size).to(torch.int) # (25600, 3)
        assert means3D_int.min() >= 0 and means3D_int[:, 0].max() < self.H and means3D_int[:, 1].max() < self.W and means3D_int[:, 2].max() < self.D
        radii = torch.ceil(scales.max(dim=-1)[0] * self.scale_multiplier / self.grid_size).to(torch.int) # (25600,)
        assert radii.min() >= 1
