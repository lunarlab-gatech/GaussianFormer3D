...
# from mmcv.runner import force_fp32, auto_fp16 # Check whether we can use them
...
class GridMaskHybrid(nn.Module):
    def __init__(self, use_h, use_w, rotate = 1, offset=False, ratio = 0.5, mode=0, prob = 1.):
        super(GridMaskHybrid, self).__init__()
    ...
    def forward(self, x, dpt=None):
        ...
        if self.offset:
            offset = torch.from_numpy(2 * (np.random.rand(h,w) - 0.5)).to(x.dtype).cuda()
            x = x.view(-1,h,w)
            mask_x = mask.expand_as(x)
            x = x * mask_x + offset * (1 - mask_x)
            returns.append(x.view(n,c,h,w))
            if not (dpt is None):
                _,c_dpt,_,_ = dpt.size()
                dpt = dpt.view(-1, h, w)
                mask_dpt = mask.expand_as(dpt)
                dpt = dpt * mask_dpt + offset * (1 - mask_dpt)
                returns.append(dpt.view(n,c_dpt,h,w))
        else:
            x = x.view(-1,h,w)
            mask_x = mask.expand_as(x)
            x = x * mask_x
