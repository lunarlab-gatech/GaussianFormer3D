...
class GaussianOccEncoder3D(BaseEncoder):
    def __init__(
        self,
        anchor_encoder: dict,
        norm_layer: dict,
    ...
    def forward(
        self,
        representation, # (B, N, 28)
        rep_features, # (B, N, 128)
        ms_img_feats=None, # should be a list of tensors
        out_dpt_multiscale=None, # should be a list of tensors
        metas=None,
        **kwargs
    ):
        ...
        dpt_feature_maps = out_dpt_multiscale
        ...
        """
        operation order listed in config:
        "deformable3d", (update the query which is the high-dimensional feature of gaussians)
        "ffn",
        "norm",
        "refine",
        ...
        for i, op in enumerate(self.operation_order):
            if op == 'spconv':
                ...
                instance_feature = self.layers[i](
                    ...
                    anchor_embed,
                    feature_maps,
                    dpt_feature_maps, # depth feature maps added
                    metas,
