...
class SparseGaussian3DRefinementModule(BaseModule):
    def __init__(
        ...
        if restrict_xyz:
            ...
            unit_prob = [unit_xyz[i] / (pc_range[i + 3] - pc_range[i]) for i in range(3)] # [0.04, 0.04, 0.125]
            unit_sigmoid = [4 * unit_prob[i] for i in range(3)] # [0.16, 0.16, 0.5]
    ...
    def forward(
        ...
        gs_scales = safe_sigmoid(output[..., 3:6]) # ranging between 0 and 1
