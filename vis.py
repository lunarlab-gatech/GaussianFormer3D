...
def save_gaussian(save_dir, gaussian, name):
    # we get totally 18 classes, the first 17 classes are non-empty classes, the last one is empty class, 0 is ignored class (check from SurroundOcc github)
    empty_label = 17
    ...
    opas_threshold = [0.01] # for final gaussians in ours

    for opa in opas_threshold:
        mask = (pred != empty_label) & (opas > opa)
